// ---------------------------------------------------------------------------
// 施設名検索用のコンパクトな索引を生成する。
//
// 施設名で地図アプリ（BA-SHARE 等）から検索できるよう、全市区町村の data.json を
// 横断して「名前・住所・座標・精度レベル」だけを抽出し、キー名の繰り返しを避ける
// 配列形式にまとめる。
//
// データ量が増え単一ファイルが GitHub の 100MB/ファイル上限に近づくため、
// **都道府県ごとに分割**して出力する:
//   api/search-index.json                 マニフェスト（都道府県一覧・件数・schema・各shardのパス）
//   api/search-index/{都道府県}.json        その都道府県の索引（{ meta, data:[...] }）
//
// 利用側は search-index.json（マニフェスト）を読み、必要な都道府県 shard を取得する。
// 全国横断検索する場合は全 shard を取得して連結する。
//
// 使い方:
...
const SHARD_DIR = path.join(ROOT, 'api', 'search-index');

const SCHEMA = ['name', 'address', 'lat', 'lng', 'level'];
...
export function generateSearchIndex() {
  ...
  // 古い shard を消してから作り直す（都道府県が減った場合の取り残しを防ぐ）。
  ...
  for (const pref of fs.readdirSync(FACILITIES_DIR).sort()) {
    ...
    for (const city of fs.readdirSync(prefDir).sort()) {
      ...
      if (json?.meta?.updated) prefUpdated = Math.max(prefUpdated, json.meta.updated);
    ...
    updated = Math.max(updated, prefUpdated);
    total += rows.length;
    const file = `search-index/${safeName(pref)}.json`;
    fs.writeFileSync(
      path.join(ROOT, 'api', file),
      JSON.stringify({ meta: { updated: prefUpdated, count: rows.length, schema: SCHEMA }, data: rows }),
    );
    prefectures.push({ name: pref, count: rows.length, file });
  }

  // マニフェスト（従来の search-index.json のパスに置く）。
  const manifest = {
    meta: { updated, count: total, schema: SCHEMA },
    prefectures,
  };
  fs.mkdirSync(path.dirname(INDEX_PATH), { recursive: true });
  fs.writeFileSync(INDEX_PATH, JSON.stringify(manifest));
  console.log(`  検索索引: ${total}件 / ${prefectures.length}都道府県 shard → api/search-index/`);
  return manifest;
}
